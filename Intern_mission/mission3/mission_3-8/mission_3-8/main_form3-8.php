...
if(!isset($_SESSION['login'])){
	header("Location:http://co-622.it.99sv-coco.com/mission_3-8/login3-8.php");
	exit();
}else{
	$name = $_SESSION['name'];
}
...
<form action="main_form3-8.php" method="post" enctype="multipart/form-data">
	<!-- フォーム作成 main_form3-8.phpに送信 -->
	*名前:<br/>
...
<a href = http://co-622.it.99sv-coco.com/mission_3-8/delete_form3-8.html>削除フォーム</a><br/>
<a href = http://co-622.it.99sv-coco.com/mission_3-8/edit_form3-8.html>編集フォーム</a><br/>
<a href = http://co-622.it.99sv-coco.com/mission_3-8/logout3-8.php>ログアウト</a><br/>
<h1>みんなのコメント</h1><br/>
		...
		<figure>
			<?php
				if(array_key_exists($result['extension'], $imgMIMETypes)){
					...
					// var_dump($mime);
					echo '<img src="contentshow3-8.php?mime='.$mime.'&id='.$result['id_at'].'"/>';
				}else if(array_key_exists($result['extension'], $vidMIMETypes)){
					$mime = $vidMIMETypes[$result['extension']];
					// var_dump($mime);
					echo '<video controls><source src="contentshow3-8.php?mime='.$mime.'&id='.$result['id_at'].'"/></video>';
				}
			?>
		</figure>
		<br/>
