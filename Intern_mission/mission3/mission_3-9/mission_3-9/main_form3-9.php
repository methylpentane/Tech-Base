...
if(!isset($_SESSION['account'])){
	header("Location:http://co-622.it.99sv-coco.com/mission_3-9/login_form3-9.php");
	exit();
}else{
...
<form action="main_form3-9.php" method="post" enctype="multipart/form-data">
	<!-- フォーム作成 main_form3-9.phpに送信 -->
	*名前:<br/>
...
<a href = http://co-622.it.99sv-coco.com/mission_3-9/delete_form3-9.html>削除フォーム</a><br/>
<a href = http://co-622.it.99sv-coco.com/mission_3-9/edit_form3-9.html>編集フォーム</a><br/>
<a href = http://co-622.it.99sv-coco.com/mission_3-9/logout3-9.php>ログアウト</a><br/>
<h1>みんなのコメント</h1><br/>
		...
		<figure>
			<?php
				if(array_key_exists($result['extension'], $imgMIMETypes)){
					...
					// var_dump($mime);
					echo '<img src="contentshow3-9.php?mime='.$mime.'&id='.$result['id_at'].'"/>';
				}else if(array_key_exists($result['extension'], $vidMIMETypes)){
					$mime = $vidMIMETypes[$result['extension']];
					// var_dump($mime);
					echo '<video controls><source src="contentshow3-9.php?mime='.$mime.'&id='.$result['id_at'].'"/></video>';
				}
			?>
		</figure>
		<br/>
